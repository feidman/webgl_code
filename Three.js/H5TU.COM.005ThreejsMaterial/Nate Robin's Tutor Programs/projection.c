...
drawstr(GLuint x, GLuint y, char* format, ...)
{
    ...
    glRasterPos2i(x, y);//光栅位置函数，设置当前所画位图或图像的原点
    for (s = buffer; *s; s++)
        glutBitmapCharacter(font_style, *s);//逐个写字符
}
...
int
cell_hit(cell* cell, int x, int y)//拣选功能
{
...
drawmodel(void)
{
    if (!pmodel) {
        ...
        if (!pmodel) exit(0);
        glmUnitize(pmodel);//通过将其移到坐标原点，放缩到原点附近的单位立方体内部，单位化模型
        glmFacetNormals(pmodel);//求面法向
        glmVertexNormals(pmodel, 90.0);//求点法向
    }
...
invert(GLdouble src[16], GLdouble inverse[16])
{
    ...
    for (i = 0; i < 4; i++) {//将一维数组转化为二维数组
        for (j = 0; j < 4; j++) {
            tmp[i][j] = src[i*4+j];
        }
    }
    
    for (i = 0; i < 4; i++) {//将矩阵化成行阶梯矩阵，线性代数？
        ...
        if (tmp[i][i] == 0) {//对角线上有元素为零，此矩阵行列式为零，是奇异矩阵
...
{
    glViewport(0, 0, width, height);//视口的概念
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluOrtho2D(0, width, height, 0);//特殊的平行投影函数，用于二维图像到二维屏幕上的投影
    glMatrixMode(GL_MODELVIEW);//指定哪一个矩阵堆栈是下一个矩阵操作的目标
    glLoadIdentity();
...
#define GAP  25             /* gap between subwindows */
    ...
    glutSetWindow(world);//令标记符为world的窗口为当前窗口
    glutPositionWindow(GAP, GAP);//改变当前窗口的位置
    glutReshapeWindow(sub_width, sub_height);//改变当前窗口的尺寸
...
main_display(void)
{
    glClearColor(0.8, 0.8, 0.8, 0.0);//清除颜色缓冲区
...
world_reshape(int width, int height)
{
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluPerspective(60.0, (GLfloat)width/height, 1.0, 256.0);//创建对称的透视视见体
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    glTranslatef(0.0, 0.0, -5.0);//平移
    glRotatef(-45.0, 0.0, 1.0, 0.0);//旋转
...
world_display(void)
{
    ...
    l[2] = lookat[5].value - lookat[2].value;//从观察坐标系原点，即眼睛的位置指向参考点的向量
    length = normalize(l);//单位化向量
    
    invert(modelview, inverse);//求逆矩阵
    ...
    glPushMatrix();
    glColor3ub(0, 0, 255);
    glBegin(GL_LINE_STRIP);//用蓝色画观察点到参考点的向量，为什么是折线？
    glVertex3f(0.0, 0.0, 0.0);
    ...
    glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, 'e');//用黄色写字符e
    ...
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);//使用源颜色的alpha值,1.0-alpha的值作因子
    glColor4f(0.2, 0.2, 0.4, 0.5);
    glBegin(GL_QUADS);//后裁剪面
    glVertex3i(1, 1, -1);
    glVertex3i(-1, 1, -1);
    glVertex3i(-1, -1, -1);
    glVertex3i(1, -1, -1);
...
command_mouse(int button, int state, int x, int y)
{
    ...
    if (state == GLUT_DOWN) {
        ...
        /* mouse should only hit _one_ of the cells, so adding up all
            ...
            selection += cell_hit(&perspective[0], x, y);//拣选，返回cell的id
